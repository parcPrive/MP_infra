...
consumer 처리 실패 메시지는 재시도 후 DLT topic으로 이동한다.
```
...
DefaultErrorHandler
  ...
  +-- 설정된 횟수만큼 retry
  |
  +-- 계속 실패하면 DeadLetterPublishingRecoverer 실행
  |
  v
원본 topic 이름 + .DLT topic으로 메시지 발행
```
...
```text
order.created 실패 -> order.created.DLT
payment.completed 실패 -> payment.completed.DLT
```
